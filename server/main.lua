...
RegisterNetEvent('qb-gopostal:server:DoBail', function(bool, vehInfo)
    local src = source
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.BailPrice then
            ...
            TriggerClientEvent('qb-gopostal:client:SpawnVehicle', src, vehInfo)
        ...
        if Bail[Player.PlayerData.citizenid] then
            Player.Functions.AddMoney('cash', Bail[Player.PlayerData.citizenid], "gopostal-bail-paid")
...
RegisterNetEvent('qb-gopostal:server:01101110', function(drops)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    Player.Functions.AddMoney("bank", payment, "gopostal-salary")
    TriggerClientEvent('QBCore:Notify', src, Lang:t("success.you_earned", {value = payment}), 'success')
end)

RegisterNetEvent('qb-gopostal:server:nano', function()
